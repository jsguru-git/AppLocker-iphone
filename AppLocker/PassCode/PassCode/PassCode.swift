...
fileprivate enum LockerConstants {
	...
	static let passCodeNib = "PassCode" // Nib name
...
enum LockerMode { // Modes for PassCode
	case validate
	case change
	case desactive
	case create
...
extension PassCode {
	...
	class func present(with mode: LockerMode, and config: LockerConfig? = nil) {
		...
		if let passCode = getPassCode(with: mode, and: config) {
			UIApplication.shared.keyWindow?.rootViewController?.present(passCode, animated: true, completion: nil)
		}
	}
	
	class func getPassCode(with mode: LockerMode, and config: LockerConfig? = nil) -> (UIViewController?) {
		
		guard let passCode = Bundle.main.loadNibNamed(LockerConstants.passCodeNib,
		                                              owner: self,
		                                              options: nil)?.first as? PassCode else {
														return nil
		}
		
		passCode.messageLabel.text = config?.title ?? ""
		
		passCode.mode = mode
		
		if let image = config?.image {
			passCode.photoImageView.image = image
		} else {
			passCode.photoImageView.isHidden = true
		}
		
		if let subtitle = config?.subtitle {
			passCode.submessageLabel.text = subtitle
		}
		
		if let color = config?.color {
			passCode.view.backgroundColor = color
		}
		return passCode
	}
}
