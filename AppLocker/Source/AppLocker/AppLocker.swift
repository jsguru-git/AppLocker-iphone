...
class AppLocker: UIViewController {
  ...
  fileprivate func clearView() {
	...
	// MARK: - SwitchBack
	fileprivate func swithBack() {
		let storyboard = UIStoryboard.init(name: "Main", bundle: nil)
		let nav = storyboard.instantiateInitialViewController() as! AppListViewController
...
extension AppLocker {
  ...
  class func present(with mode: LockerMode, and config: LockerConfig? = nil) {
		...
		if let appLocker = getAppLocker(with: mode, and: config) {
      UIApplication.shared.keyWindow?.rootViewController?.present(appLocker, animated: true, completion: nil)
		}
  }
	
	class func getAppLocker(with mode: LockerMode, and config: LockerConfig? = nil) -> (UIViewController?) {
		
		guard let appLocker = Bundle.main.loadNibNamed(LockerConstants.appLockerNib,
		                                            owner: self,
		                                            options: nil)?.first as? AppLocker else {
			return nil
		}
